...
__global__ void cudaBFS(int num_nodes, const int *offsets,
                        const int *destinations, int *Fa, int *Fa_len, int *Sa,
  ...
  */
  ...
  for (int u = 0; u < NUM_BFS; u++) { // choose source u
    ...
    __syncthreads(); // without this, Sa[source[u]] might be overwrite to -1,
                     // Fa[0] should be ok since tid[0] must
    if (tid == 0) {
      *Fa_len = 0;
      Sa[source[u]] = 0;
      Fa[*Fa_len] = source[u];
    ...
    while (*Fa_len != 0) {
      ...
      for (int v = tid; v < *Fa_len; v += blockDim.x) {
        ...
        for (int w_idx = start; w_idx < end;
             w_idx++) { // each thread for one frontier
      ...
      __syncthreads(); // without this, next level setArray is call before this
                       // level complete.
    }
    if (tid == 0) {
      sink[u] = Fa[0];
...
int main(int argc, char **argv) {
  ...
  if (argc != 5) {
    std::cerr
        << "Usage: " << argv[0]
        << " <csr_binary_file> <out_file_path> <start_vertex> <end_vertex>"
        << std::endl;
    return 1;
  }
  ...
  if (csr.loadFromBinary(csr_binary_file)) {
    std::cout << "Loading " << txt_file << " from binary file..." << std::endl;
  } else {
  ...
  for (start_vertex = std::stoi(argv[3]); start_vertex < 10;
       start_vertex += 1000) {
    end_vertex = start_vertex + NUM_BFS - 1;
    std::cout << "On-going.." << int((start_vertex / 10.0) * 100.0)
              << " %\r";
    std::cout.flush();
    ...
    CHECK(cudaMemcpy(d_source, h_source_vector.data(), NUM_BFS * sizeof(int),
                     cudaMemcpyHostToDevice));
    ...
    cudaBFS<<<NUM_BLOCKS, BLOCK_SIZE>>>(
        csr.num_nodes, d_offsets, d_destinations, d_frontier_queue,
        d_queue_length, d_status_array, d_source, d_sink, d_distance);
    timer.stop();
    ...
    // write the result
    std::string filename = std::string(argv[2]) + "/output_" +
                           std::to_string(start_vertex) + "_" +
                           std::to_string(end_vertex) + ".txt";
    ...
    for (int i = 0; i < NUM_BFS; i++) {
      outfile << "source = " << h_source[i] << ", sink = " << h_sink[i]
              << ", distance = " << h_distance[i] << std::endl;
    }
